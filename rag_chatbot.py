...
{self.config.system_prompt}
    ...
    def query_stream(self, question: str, top_k: Optional[int] = None):
        """Query the RAG system with streaming response and source references."""
        ...
        if getattr(self, "llm_backend", "openai") == "mock":
            ...
            dummy_source = [{
                "doc_id": "mock-doc",
                "text": "This is the mock document used for testing.",
                "metadata": {"title": "Mock Page", "source": "mock.md"}
            }]
            # Stream dummy response if mock backend is set
            for token in ["This ", "is ", "a ", "mock ", "response."]:
                yield token, dummy_source
            return   
        # else:
